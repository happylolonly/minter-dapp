...
async function loadInfo() {
  ...
  const price = web3.utils.fromWei(info[2][2], "ether");
...
function setTotalPrice() {
  ...
  const totalPriceWei = BigInt(info[2][2]) * BigInt(mintInputValue);
...
async function mint() {
  ...
  const value = BigInt(info[2][2]) * BigInt(amount);
  const publicMintActive = await contract.methods.mintingActive().call();
  const presaleMintActive = await contract.methods.presaleActive().call();
  ...
  if (publicMintActive) {
    // PUBLIC MINT
    try {
      const mintTransaction = await contract.methods.mint(amount).send({ from: window.address, value: value.toString() });
      if (mintTransaction) {
        if (chain === "rinkeby") {
